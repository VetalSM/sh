            ...
            <div class="flex flex-col w-full p-8 text-gray-800 bg-white shadow-lg pin-r pin-y md:w-4/5 lg:w-4/5">
                ...
                <div class="flex-1">
                    <table class="w-full md:table-fixed " cellspacing="0">
                        <thead>
                        <tr class="h-12 uppercase text-2xl lg:text-sm">
...
{{--                                        <span class=" font-bold">{{$price->weight.'г'.' ('.$item->price.' грн'.')' }}</span>--}}
                                ...
                                <td class="justify-center mt-8 md:justify-end md:flex">
                                    <div class="h-10 w-28">
                                        <div class="relative text-2xl lg:text-sm flex flex-row w-full h-8">
                                ...
                                <span class="ttext-2xl lg:text-sm font-medium lg:text-base  max-height">
                                    {{ $item->price*$item->quantity }} грн
                                </span>
                                </td>
                                <td class="text-right text-2xl lg:text-sm md:table-cell " style="padding: 0px 25px 4px 0;">
                                    <form action="{{ route('cart.remove') }}" method="POST">
                                        @csrf
                                        <input type="hidden" value="{{ $item->id }}" name="id">
                                        <button class="px-3 text-2xl lg:text-sm py-0.5 text-white bg-red-600  rounded-full">x</button>
                                    </form>
                    ...
                    <div>
                        ...
                        <form action="{{route('order')}}" method="post">
                            @csrf
                            <div class="form-group text-2xl lg:text-sm">
                                {{--@dd($cartItems)--}}
